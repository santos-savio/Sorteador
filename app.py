...
O estado e mantido em memoria e persistido em data/state.json, o que e
suficiente para um app local de um unico usuario.
"""
...
def default_config() -> dict:
    return {
        "spinDurationMs": 6000,   # tempo do giro em milissegundos
        "mode": "repeat",          # "repeat" ou "unique"
        "shuffleOnSpin": False,    # embaralhar antes de cada giro
        "sound": True,             # tocar som no giro
        "title": "Sorteador",
    }
...
def pick_winner() -> int | None:
    """Escolhe um indice vencedor respeitando pesos e modo. Retorna None se nao houver candidatos."""
...
def api_set_entries():
    ...
    for i, item in enumerate(raw):
        if isinstance(item, str):
            ...
            if not text:
                continue
            # preserva customizacao/flag se o texto ja existia
...
# Carrega o estado assim que o modulo e importado (cobre `flask run` e gunicorn).
load_state()
